...
tracking inventory:  fw/<FW>/docs/gme-callgraph-inventory.{md,tsv}

Source of truth for the "name+signature+docstring the whole GME region" campaign. Reads the
CURRENT named decompilation (out/decomp_named) to build the caller->callee graph, BFS from the
...
Usage:  FW=fw/2N-update3202MT python3 tools/gme_inventory.py   (default FW=fw/2N-update3202MT)
Re-run after every regen to refresh the inventory.
"""
import os, re, collections

FW = os.environ.get("FW", "fw/2N-update3202MT")
ROOT = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
FWD = FW if os.path.isabs(FW) else os.path.join(ROOT, FW)
DEC = os.path.join(FWD, "out/decomp_named")
...
hdr = ("# GME-format / script-engine call-graph inventory\n\n"
 "Auto-generated by `tools/gme_inventory.py` from `out/decomp_named` call edges + "
