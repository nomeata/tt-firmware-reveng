#!/usr/bin/env bash
# PURE regen:  $FW/out/decomp_named/ = f( $FW/data/PROG.bin [via $FW/ghidra/base],
...
# ATOMIC + FAIL-LOUD: decompile into decomp_named.new; swap into place ONLY on verified completion,
...
echo "[2/2] names/structs/docstrings/globals + decompile -> decomp_named.new ..."
...
if echo "$out" | grep -q 'DECOMP_NAMED DONE'; then
  ...
  echo "FAIL: decompile incomplete -- decomp_named left UNTOUCHED"; echo "$out" | tail -15
